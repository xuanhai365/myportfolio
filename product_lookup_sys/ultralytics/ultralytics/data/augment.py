...
class RandomLighting:
    def __init__(self, local_mask=(120, 160), global_mask=(30, 50), \
                 flip_and_noise=False, high_ratio=(1,2), low_ratio=(0.1, 0.5), \
                    left_low_ratio=(0.4,0.6), left_high_ratio=(0,0.2), right_low_ratio=(0.4,0.6), \
                        right_high_ratio = (0,0.2), p = 0.5):
    ...
    def __call__(self, labels):
        if random.random() < self.p:
            ...
            if random.random() < 0.5:
                augmenter = Ill_Aug(self.local_mask, self.global_mask, self.flip_and_noise, 1)
                img = augmenter.augment_image(img)[0]
            else:
                augmenter = RandomShadows(1, self.high_ratio , self.low_ratio, self.left_low_ratio, self.left_high_ratio, self.right_low_ratio, self.right_high_ratio)
                img = augmenter(img)
            labels['img'] = img
        return labels
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                A.Blur(p=0.1, blur_limit= (5, 9)),
                A.ToGray(p=0.1),
                A.CLAHE(p=0.0),
                A.RandomBrightnessContrast(p=0.0),
                A.RandomGamma(p=0.0),
                A.ImageCompression(quality_lower=75, p=0.0),
                A.MotionBlur(p=0.1, blur_limit= (5, 9))]  # transforms
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose([
        ...
        RandomLighting(p = 0.3)])  # transforms
